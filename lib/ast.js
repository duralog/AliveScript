...
exports.Class = Class = (function(superclass){
  ...
  prototype.compile = function(o, level){
    var fun, body, lines, title, boundFuncs, curriedBoundFuncs, decl, name, proto, ctorName, ctor, ctorPlace, protoProps, importProtoObj, i$, len$, i, node, f, vname, args, that, imports, ref$, res$, clas;
    ...
    importProtoObj = function(node, i){
      var j, prop, key, pp, k, i$, ref$, len$, v;
      j = 0;
      for (; j < node.items.length; j++) {
        prop = node.items[j];
        key = prop.key;
        if (key && (pp = protoProps[k = key.name || key.value]) && protoProps.hasOwnProperty(k)) {
          if (typeof pp.carp === 'function') {
            pp.carp("duplicate prototype member '" + k + "'");
          } else {
            throw new Error("duplicate prototype member... '" + k + "'");
          }
        } else if (!protoProps.hasOwnProperty(k)) {
          protoProps[k] = node.items[j];
        }
